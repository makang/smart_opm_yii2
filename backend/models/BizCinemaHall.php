...
class BizCinemaHall extends \yii\db\ActiveRecord
{
	...
	{
		return 'bis_cinema_hall';
	}
    public static function model($className=__CLASS__){
        return new $className;
    }

    public function aGetIdByBizCinemaNos($cinemaNo,$hallNo){
        $ret = array();
        $list = $this::find()->select('id,cinema_no,hall_no')->where(['cinema_no'=>$cinemaNo,'hall_no'=>$hallNo])->asArray()
            ->all();
        foreach($list as $v){
            $ret = $v['id'];
        }
