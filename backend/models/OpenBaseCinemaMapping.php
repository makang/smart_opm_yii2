...
class OpenBaseCinemaMapping extends \yii\db\ActiveRecord
{
	...
	{
		return 'open_basebis_cinemamapping';
	}
    public static function model($className=__CLASS__){
        return new $className;
    }

    public function aGetMappingByCinemas($cinemaNos){
        $ret = array();
        $list = $this::find()->select('BisCinemaNo,BaseCinemaNo')->where(['BaseCinemaNo'=>$cinemaNos])->orderBy('CreateTime asc')->asArray()
            ->all();
        foreach($list as $v){
            $ret[$v['BaseCinemaNo']] = $v['BisCinemaNo'];
        }
        return $ret;
    }
